...
async fn run(cli: Cli) -> eyre::Result<()> {
    ...
    // Translation: `translate_target` always carries the target (including
    // "en"); `use_native_translation` asks the model to translate natively when
    // it supports the (source, target) pair, otherwise the engine falls back to
    // a Google Translate post-pass. Mirrors the Tauri backend
    // (transcription_api.rs) after the engine dropped `whisper_to_english`.
    if cli.translate {
        let target = cli
            .target_language
            .clone()
